...
Validator.prototype.validate = function(values, presentOnly, cb) {
  ...
  function validate(validation, cb) {
    ...
    async.each( Object.keys(requirements.data), function (key, cb) {
      ...
      if(requirements.data.criteria){
        if(_.isArray(values[validation])){
          values[validation] = waterlineCriteria(values[validation], requirements.data.criteria).results;
        }else{
          values[validation] = waterlineCriteria([values[validation]], requirements.data.criteria).results[0] || null;
        }
        //deletion of criteria to not cause anchor to fail
        delete requirements.data.criteria;
      }
  ...
  async.each(validations, validate, function allValidationsChecked () {
    var validated = waterlineCriteria([values], self.criteria).results[0];
    if(!validated){
      errors.Criteria = [{message : 'Object :\n'+require('util').inspect(values)+' does not respect criteria defined in the model.'}];
    }
