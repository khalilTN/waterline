...
Validator.prototype.validate = function(values, presentOnly, cb) {
  ...
  function validate(validation, cb) {
    ...
    async.each( Object.keys(requirements.data), function (key, cb) {
      ...
      if(requirements.data.criteria){
        values[validation] = waterlineCriteria(value, requirements.data.criteria).results;
        //deletion of `requirements.data.criteria` to not cause anchor validation to fail
  ...
  async.each(validations, validate, function allValidationsChecked () {
    ...
    if(!validated){
      errors.Criteria = [{message : 'Object does not respect criteria defined in the model'}];
    }
