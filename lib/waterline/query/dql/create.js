...
var filter = require('waterline-criteria');
...
module.exports = function(values, cb) {
  ...
  var defaultCriteria = this.waterline.schema[this.identity].criteria;
  if(_.isArray(values)){
    values = filter(values, defaultCriteria).results;
  }
  else {
    values = filter([values], defaultCriteria).results[0];
  }
